...
#maryland_zcta <- read_csv('data/baltimore_911.csv') |> clean_names() 
 
maryland_zcta <- maryland_zcta |> mutate(zcta5n = as.character(zcta5n))

#baltimore_911 <- read_csv('data/maryland_zcta.csv') |> clean_names()

baltimore_911 <- baltimore_911 |> mutate(zip_code = as.character(zip_code))
```

```{r}
baltimore_combined <- baltimore_911 |> left_join (maryland_zcta, join_by (zip_code == zcta5n))
...
**A1. Carroll has the most, followed by Cecil and Allegany.**

```{r}
combined_911 |> group_by (county) |>
...
**A2. For the first part of the question, rural counties like Allegany and Carroll had the most calls and calls seemed to peak in the summer. For the second part: 314 Grove Neck RD in Cecil County had the most calls – twice as many as the next highest on the list.**

```{r}
combined_911 <- combined_911 |>
  mutate(month = month(date))

combined_911 |> group_by(county, month) |>
  summarise (total_calls = sum(n())) |>
  arrange(desc(total_calls))

combined_911 |> group_by(county, location) |>
  summarise (total_calls = sum(n())) |> 
  arrange(desc(total_calls))
```
...
**A3. The top zip code – 21217 – is in west Baltimore. It had 774 calls. The same zip code also had the highest calls per capita, at nearly 21. This includes the neighborhoods of Sandtown-Winchester and Druid Hill Park.**

```{r}
under_18 <- baltimore_combined |> filter(pct_under_18 >= 75)

under_18 |> group_by(population, zip_code) |>
  summarise(total_calls = sum(n())) |>
  arrange(desc(total_calls))

under_18 |> group_by (population, zip_code) |> 
  summarise (people = (sum(population)/n()))
...
**A4: Upton had 201 calls, the most calls by a significant margin.**

```{r}

baltimore_combined |> group_by(neighborhood) |> summarise (total_calls = sum(n())) |>
  arrange (desc(total_calls))
```
